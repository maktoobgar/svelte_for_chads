<script lang="ts">
	...
	export let name: string = $LL.Table.Users();
	export let description: string = $LL.Table.Description();
		...
		{#if addButton}
			<div class="flex items-center mt-4 gap-x-3">
				...
				>
					...
					<span class="ltr:capitalize">{$LL.Table.Add()}</span>
				</Button>
			</div>
		{/if}
	</div>
	...
	<div class="flex flex-col mt-6">
		<div class="-mx-4 -my-2 sm:-mx-6 lg:-mx-8 overflow-x-auto">
			<div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
				<div
					class={`relative overflow-hidden rounded-[16px] shadow-custom dark:shadow-custom-dark bg-white dark:bg-gray-900 ${status === 'loading' && 'min-h-[200px]'}`}
				>
					<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
							...
							<tr>
								{#each columns as col (col.id)}
									...
									>
										...
										>
											<span class="ltr:first-letter:capitalize">{col.name}</span>
						...
						{#if status === 'idle' || status === 'success'}
							<tbody
								...
								in:fade={{ duration: 150, delay: 160, easing: easeOut }}
											...
											>
												...
												{:else if col.type === 'progressBar'}
													<div class="w-48 h-1.5 bg-blue-200 overflow-hidden rounded-full">
														<div
															style={`width: ${item.data[col.colName].content}%;`}
					...
					{#if status === 'loading'}
						<h2
							...
							in:fly={{ y: 20, duration: 150, delay: 160, easing: easeOut }}
